...
void DoubleList::add_song(const string& ruta_archivo, Data*& list) {
    Data* new_song = getMetaData(const string& ruta_archivo);
    if (!list) {
        list = new_song;
    } else {
        Data* temp = list;
        ...
        temp->siguiente = new_song;
    }
}

Data* DoubleList::getMetaData(string ruta_archivo) {
    TagLib::FileRef archivo(ruta_archivo.c_str());
    if (!archivo.isNull() && archivo.tag()) {
        TagLib::Tag *tag = archivo.tag();
        if (tag) {
            Data* new_song = create_song(tag);
            return new_song
        }
    }
}

Data* DoubleList::create_song(TagLib::Tag tag){
    char nombre[64];
    char artista[64];
    char album[64];
    char path[128];
    ...
    Data *new_song = new Data(path, nombre, artista, album, minutos, segundos, 0);
    ...
    for (const auto& entry : fs::directory_iterator(ruta_carpeta)) {
        if (entry.is_regular_file()) {
            ...
            if (extension == ".mp3" || extension == ".MP3") {
                ...
                getMetaData(entry.path().string(), lista);
            }
        }
    }
}
